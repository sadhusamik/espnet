...
class TrainerOptions:
    ...
    stop_frontend_updates_after_epoch: int
...
class Trainer:
    ...
    def run(
        ...
        if trainer_options.stop_frontend_updates_after_epoch == -1:
            frontend_check = False
            logging.info('Front-end training (if on) will not be stopped')
        else:
            frontend_check = True
            logging.info('Front-end training (if on) will be stopped before starting epoch {:d}'.format(
                trainer_options.stop_frontend_updates_after_epoch))
        ...
        for iepoch in range(start_epoch, trainer_options.max_epoch + 1):
            ...
            if frontend_check:
                if iepoch >= trainer_options.stop_frontend_updates_after_epoch:
                    frontend_check = False
                    logging.info('Front-end training STOPPED!')
                    if distributed_option.distributed:
                        if trainer_options.sharded_ddp:
                            logging.info('No idea what to do....')
                            # TODO: Figure this out
                        else:
                            model = dp_model.module
                            model.frontend.fdlp_spectrogram.updatable_params.requires_grad_(False)
                            dp_model = torch.nn.parallel.DistributedDataParallel(
                                model,
                                device_ids=(
                                    # Perform multi-Process with multi-GPUs
